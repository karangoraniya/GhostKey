...
export function DemoFlow({ d }: { d: Dashboard }) {
  ...
  return <div className="demo-stages">
    <details className={`agent-disclosure ${d.demoUsable ? "has-agent" : ""}`} open={!d.demoUsable}><summary>✓ {d.demoGhost?.name ?? "Agent"} is ready <span>View identity and permissions</span></summary>
    <Card id="demo-agent" title="Create Agent" subtitle="Give it a name. We’ll handle the temporary permissions." icon={<span className="stage-number">01</span>} className={`demo-stage ${d.demoUsable ? "stage-completed" : "stage-current"}`} aside={!d.demoGhost ? <StatusBadge tone="warning" pulse>START HERE</StatusBadge> : undefined}>
    ...
    <div hidden={!d.demoUsable}><Card id="demo-execute" title="Run Agent" subtitle="Create a real GitHub issue without handing the credential to the agent." icon={<span className="stage-number">02</span>} className={`demo-stage ${d.demoUsable && !d.demoBlocked ? "stage-current" : ""} ${!d.demoUsable ? "stage-locked" : ""}`} aside={<span className="section-number">FIRST PROVIDER ADAPTER · GITHUB</span>}>
      ...
      <form onSubmit={event => { event.preventDefault(); const data = new FormData(event.currentTarget); void d.provider("issue", { title: String(data.get("title")), body: String(data.get("body")) }, true); }}>
        ...
        <details className="task-details"><summary>Edit issue description</summary><label className="mt-3">Issue body<textarea name="body" rows={2} defaultValue="Created by an ephemeral GhostKey agent using scoped authority." maxLength={10000} /></label></details>
      ...
      <details className="demo-boundary" open={succeeded}><summary>Try an action outside its permissions</summary><h3>Test the boundary</h3><p>What happens when the same agent asks for authority it does not have?</p><button className="button danger mt-4" disabled={Boolean(d.busy) || !d.demoUsable} onClick={() => void d.provider("blocked", undefined, true)}>{d.busy === "demo-blocked" ? "Checking boundary…" : "Test Unauthorized Action"}</button>
    ...
    <div hidden={!d.demoBlocked || !d.demoUsable}><ApprovalPanel d={d} demo /></div>
  </div>;
}
